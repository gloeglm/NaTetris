...
public class InfoPanel extends JPanel {
  ...
  /**
   * Constants used through the panel for layout
   */
  ...
  protected void paintComponent(Graphics g) {
     ...
     * the piece itself 
     */
    g.drawRect(0, 0, predictionSquareWidth(), predictionSquareWidth());
      ...
      for (int x = 0; x < dim; x++) {
        for (int y = 0; y < dim; y++) {
          if (piece.isTile(x, y, 0)) {
        	  int currX = PADDING_5 + (x * TILE_SIZE);
        	  int currY = PADDING_5 + (y * TILE_SIZE);
    ...
    offset += predictionSquareWidth(); // strings have to be underneath the prediction square to be visible
  ...
  private int predictionSquareWidth() {
    return Board.TILE_SIZE * MAX_PIECE_DIMENSION + PADDING_5;
  }
